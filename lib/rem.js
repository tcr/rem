...
var Route = (function () {
  ...
  function Route (url, defaultBodyMime, redirect, callback) {
    this.url = url;
...
var Client = (function () {
  ...
  function Client (options) {
    ...
    // Default options.
    this.options = { format: 'json', redirect: true };
    if (options) {
  ...
  Client.prototype._request = function (format /*, segments... */) {
    ...
    var url = ((segments[0] || '').indexOf('//') != -1 ? segments.shift() : (segments.length ? '/' : ''))
      + (segments.length ? env.joinPath.apply(null, segments.map(String)) : '');
    url = new URL(url);
    augment(url.query, query);

    return new Route(url, this.options.uploadFormat, this.options.redirect, function (req, routeMiddleware, next) {
      var stream = new CrossStream();

      // Disambiguate between MIME type and string body in route invocation. (ie: .get(bodyOrMime, ...))
